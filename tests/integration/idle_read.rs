use bevy::prelude::*;
use bevy_event_bus::{EventBusReader, EventBusPlugins};
...
fn idle_empty_topic_poll_does_not_block() {
    ...
    let mut app = App::new();
    app.add_plugins(EventBusPlugins(backend, bevy_event_bus::PreconfiguredTopics::new([topic.clone()])));
    #[derive(Resource, Default)] struct Ticks(u32);
    ...
    app.add_systems(Update, move |mut r: EventBusReader<TestEvent>, mut ticks: ResMut<Ticks>| {
    ...
    assert!(elapsed_ms < 1500, "Idle polling took too long: {elapsed_ms}ms");
}
