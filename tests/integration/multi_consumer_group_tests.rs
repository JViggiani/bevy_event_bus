...
async fn test_create_consumer_group() {
    ...
    let mut backend = KafkaEventBusBackend::new(kafka_connection_for_tests(
        &bootstrap,
        Some("test_client_create_group".to_string()),
    ));
...
async fn test_receive_serialized_with_group() {
    ...
    let mut backend = KafkaEventBusBackend::new(kafka_connection_for_tests(
        &bootstrap,
        Some("test_client_receive_group".to_string()),
    ));
...
async fn test_enable_manual_commits_and_commit_offset() {
    ...
    let mut backend = KafkaEventBusBackend::new(kafka_connection_for_tests(
        &bootstrap,
        Some("test_client_manual_commits".to_string()),
    ));
    ...
    assert!(
        result.is_ok(),
        "Failed to enable manual commits: {:?}",
        result.err()
    );
...
async fn test_get_consumer_lag() {
    ...
    let mut backend = KafkaEventBusBackend::new(kafka_connection_for_tests(
        &bootstrap,
        Some("test_client_lag".to_string()),
    ));
...
async fn test_multiple_consumer_groups_independence() {
    ...
    let mut backend = KafkaEventBusBackend::new(kafka_connection_for_tests(
        &bootstrap,
        Some("test_client_multi_groups".to_string()),
    ));
    ...
    backend
        .create_consumer_group(&[topic.clone()], &group_id2)
        .await
        .unwrap();
...
async fn test_consumer_group_with_multiple_topics() {
    ...
    let mut backend = KafkaEventBusBackend::new(kafka_connection_for_tests(
        &bootstrap,
        Some("test_client_multi_topics".to_string()),
    ));
