...
fn run_throughput_test(
    ...
    fn receiver_system(
        ...
        for wrapper in reader.read(&KafkaConsumerConfig::new("localhost:9092", "test_group", [&state.test_topic])) {
            ...
            if state.messages_received.len() >= state.messages_to_send as usize
                ...
                println!("Finished receiving {} messages", state.messages_received.len());
            }
        }
    }
    ...
    let receive_throughput_mb = (total_bytes_received as f64 / 1_000_000.0) / receive_duration.as_secs_f64();
    ...
    assert_eq!(state.messages_sent, message_count, "Not all messages were sent");
    assert_eq!(state.messages_received.len(), message_count as usize, "Not all messages were received");
    
    // Verify message ordering and content
    state.messages_received.iter().enumerate().for_each(|(i, msg)| {
        assert_eq!(msg.sequence, i as u32, "Message sequence mismatch at index {}", i);
        assert_eq!(msg.payload.len(), payload_size, "Payload size mismatch at index {}", i);
    });
...
fn record_performance_results(
    ...
    let csv_path = std::env::var("BENCH_CSV_PATH").unwrap_or_else(|_| "event_bus_perf_results.csv".into());
    ...
    if need_header {
        file.write_all(header.as_bytes()).expect("Failed to write CSV header");
    }
    
    file.write_all(record.as_bytes()).expect("Failed to write CSV record");
