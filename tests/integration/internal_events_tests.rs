...
fn test_internal_only_events() {
    ...
    let topic = unique_topic("internal_test_topic");
    app.add_bus_event::<TestEvent>(&topic);

    // Create test events to send
    let test_events = vec![
        TestEvent {
            message: "Internal Event 1".to_string(),
            value: 10,
        },
        TestEvent {
            message: "Internal Event 2".to_string(),
            value: 20,
        },
        TestEvent {
            message: "Internal Event 3".to_string(),
            value: 30,
        },
    ];
    ...
    let consumer_group = unique_consumer_group("internal_test_topic");
    app.add_systems(
        Update,
        move |mut writer: EventBusWriter<TestEvent>, mut sent: Local<bool>| {
            if !*sent {
                *sent = true;
                ...
                info!(
                    "Sent {} events (both external mock and internal)",
                    events_to_send.len()
                );
            }
        },
    ...
    app.add_systems(
        Update,
        move |mut reader: EventBusReader<TestEvent>, mut received: ResMut<ReceivedEvents>| {
            for event_wrapper in reader.read(&kafka_consumer_config(
    ...
    for i in 0..10 {
        ...
        info!(
            "Update cycle {}: {} internal events received",
            i,
            received.0.len()
        );
    ...
    assert_eq!(
        received.0.len(),
        test_events.len(),
        "Should receive all {} internal events",
        test_events.len()
    );
    ...
    for (i, event_wrapper) in received.0.iter().enumerate() {
        ...
        assert!(
            event_wrapper.is_internal(),
            "Event {} should be internal",
            i
        );
        assert!(
            !event_wrapper.is_external(),
            "Event {} should not be external",
            i
        );
        assert!(
            event_wrapper.metadata().is_none(),
            "Internal event {} should have no metadata",
            i
        );

        // Verify event data using Deref trait
        assert_eq!(
            event_wrapper.message, test_events[i].message,
            "Event {} message should match",
            i
        );
        assert_eq!(
            event_wrapper.value, test_events[i].value,
        ...
        assert_eq!(
            extracted_event.message, test_events[i].message,
            "Extracted event {} message should match",
            i
        );
        assert_eq!(
            extracted_event.value, test_events[i].value,
            "Extracted event {} value should match",
            i
        );
    }
...
fn test_internal_events_with_headers() {
    ...
    // Register our test event
    let topic = unique_topic("internal_headers_test_topic");
    app.add_bus_event::<TestEvent>(&topic);
    ...
    // System to read events and filter for internal ones only
    let topic_clone = topic.clone();
    let consumer_group = unique_consumer_group("internal_headers_test");
    app.add_systems(
        Update,
        move |mut reader: EventBusReader<TestEvent>, mut received: ResMut<ReceivedEvents>| {
            for event_wrapper in reader.read(&kafka_consumer_config(
    ...
    assert_eq!(
        received.0.len(),
        1,
        "Should receive exactly one internal event"
    );
...
fn test_internal_events_into_parts() {
    ...
    // Register our test event
    let topic = unique_topic("internal_parts_test_topic");
    app.add_bus_event::<TestEvent>(&topic);
    ...
    // System to read events and filter for internal ones only
    let topic_clone = topic.clone();
    let consumer_group = unique_consumer_group("internal_parts_test");
    app.add_systems(
        Update,
        move |mut reader: EventBusReader<TestEvent>, mut received: ResMut<ReceivedEvents>| {
            for event_wrapper in reader.read(&kafka_consumer_config(
