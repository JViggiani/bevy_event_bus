...
impl DecoderRegistry {
    ...
    pub fn get_decoders(&self, topic: &str) -> &[Box<dyn DecoderFn>] {
        self.topic_decoders.get(topic).map(|v| v.as_slice()).unwrap_or(&[])
    }
    ...
    pub fn decode_all(&mut self, topic: &str, data: &[u8]) -> Vec<DecodedEvent> {
        ...
        if let Some(decoders) = self.topic_decoders.get(topic) {
            for decoder in decoders {
                let decoder_name = decoder.name();
                let stats = self.decoder_stats.entry(decoder_name.to_string()).or_default();
                stats.attempts += 1;
...
mod tests {
    ...
    fn test_multi_decoder_success() {
        ...
        assert_eq!(results.len(), 1, "Expected 1 decoded result, got {}", results.len());
        
        // Debug: verify the actual event content by downcasting without type check
        let decoded_any = &results[0].event;
        println!("Can downcast to PlayerMove: {}", decoded_any.downcast_ref::<PlayerMove>().is_some());
        ...
        println!("Expected TypeId: {:?}", std::any::TypeId::of::<PlayerMove>());
        ...
        let decoded_move = results.into_iter().next().unwrap().downcast::<PlayerMove>().unwrap();
    ...
    fn test_multi_decoder_multiple_success() {
        ...
        registry.register_decoder("test_topic", TypedDecoder::new(
            |_| Some(PlayerMove { x: 1.0, y: 2.0 }),
            "custom_move_decoder"
        ));
        registry.register_decoder("test_topic", TypedDecoder::new(
            |_| Some(PlayerAttack { target: "test".into(), damage: 100 }),
        ...
        let move_result = results.iter().find(|r| r.decoder_name == "custom_move_decoder").unwrap();
        let attack_result = results.iter().find(|r| r.decoder_name == "custom_attack_decoder").unwrap();
